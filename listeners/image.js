...
const imageExtensionDictionary = {
    'png': 'image/png',
    'jpg': 'image/jpeg',
};
...
const getImage = function (req, res) {
    ...
    for (let extension of Object.keys(imageExtensionDictionary)) {
    ...
    if (!tested) return res.status(400).send({error: `The passed string is not for an image!`, fix: `Please pass a string with an ${Object.keys(imageExtensionDictionary).join(" or ")}  extension.`});

    let fileExists = fs.existsSync(fileDirectory);
    if (fileExists) {
        res.set("Content-Type", imageExtensionDictionary[fileExtension])
